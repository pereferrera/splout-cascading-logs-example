...
/**
 * Generate fake logs conforming to the Apache "access.log" format.
 * The pages of the fake website contain a "category" in the url: category/page.html
 */
public class ApacheAccessLogGenerator {
	...
	public static void generate(String out, int nUsers, int nPagesPerUser, int nDays, int nPagesInSystem, int nCategories) throws IOException {
		...
		for(int user = 0; user < nUsers; user++) {
			...
			for(int day = 0; day < nDays; day++) {
				for(int pageView = 0; pageView < nPagesPerUser; pageView++) {
					...
					String request = "GET /category" + randomNumber(nCategories) + "/" + randomNumber(nPagesInSystem) + ".html HTTP/1.1\" 200 600 \"-\" \"" + NAV + "\"";
	...
	public static void main(String[] args) throws IOException {
		ApacheAccessLogGenerator.generate("access.log", 100, 15, 10, 100, 10);
	}
}
